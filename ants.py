...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while newplace != hive:
            if self.min_range <= transition <= self.max_range and newplace.bees:
            ...
            while current_place:
                if current_place.ant:
                    if current_place.ant.container and current_place.ant.ant:
                        ...
                        if current_place.ant.ant not in self.buffed_ants:
                            if not hasattr(current_place.ant.ant, 'true_queen'):
                                current_place.ant.ant.damage *=2
                                self.buffed_ants.append(current_place.ant.ant)
                    
                    elif not hasattr(current_place.ant, 'true_queen'):
                        if current_place.ant not in self.buffed_ants and current_place.ant:
                            current_place.ant.damage *=2
                            self.buffed_ants.append(current_place.ant)
                current_place = getattr(current_place, 'exit')
        else:
